...
如：RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);//使能PA端口时钟
...
该寄存器的复位值为 0X4444 4444，复位值其实就是配置端口为浮空输入模式。从上图还可以得出： STM32 的 CRL 控制着每组 IO 端口（ A~G）的低 8 位的模式。每个 IO 端口的位占用 CRL 的 4 个位，高两位为 CNF，低两位为 MODE。
...
相关设置如下：
Use Simulator，即使用软件仿真。选择： Run to main()，即跳过汇编代码，直接跳转到 main 函数开始仿真。 设置下方的： Dialog DLL 分别为： DARMSTM.DLL
和 TARMSTM.DLL，Parameter 均为： -pSTM32F103RC，用于设置支持 STM32F103RC 的软硬件仿真。如下图：
...
我们进行软件仿真(请先确保 Options for Target Debug 选项卡里面已经设置为 UseSimulator)。开始仿真，接着显示逻辑分析窗口，点击 Setup，新建两个信号
PORTA.8 和 PORTD.2，Display Type 选择 bit，然后单击 Close 关闭该对话框，可以看到逻辑分析窗口出来了两个信号，接着点击运行。 Gird 要调节到 0.5s 左右比较合适，可以通过 Zoom 里面的 In 按钮来放大波形，通过 Out 按钮来缩小波形，或者按 All 显示全部波形。从上图中可以看到 PORTA.8 和 PORTD.2
交替输出，周期可以通过中间那根红线来测量。
...
5.[有关推挽输出、开漏输出、复用开漏输出、复用推挽输出以及上拉输入、下拉输入、浮空输入、模拟输入区别](http://blog.sina.com.cn/s/blog_6ebd49350100pzju.html)
